...
class ConfluenceClient:
    ...
    def extract_github_links(self, content: str) -> List[str]:
        """
        Extract GitHub repository links from page content.

        Args:
            content: HTML content from Confluence page.

        Returns:
            List of GitHub repository URLs found in the content.
        """
        ...
        if unique_links:
            logger.debug(f"Found {len(unique_links)} GitHub links in content")

        return unique_links

    def get_pages_with_github_links(self) -> List[Dict[str, Any]]:
        """
        Retrieve all pages and extract GitHub repository links.

        Returns:
            List of dictionaries containing page info and associated GitHub links.
        """
        logger.info("Retrieving pages with GitHub links")
        pages_content = self.get_all_pages_content()
        pages_with_links = []

        for page in pages_content:
            github_links = self.extract_github_links(page["content"])
            if github_links:
                page["github_links"] = github_links
                pages_with_links.append(page)
                logger.debug(
                    f"Page '{page['title']}' has {len(github_links)} GitHub link(s)"
                )

        logger.info(f"Found {len(pages_with_links)} pages with GitHub links")
